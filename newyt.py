...
# YOUR CHANNEL NAMES 
# 1 UCisWtz5BPyiTV8VG6OkpNoA -- Mercyonair(Mercy John)
# 2 FriesWithPotate
# 3 MicsOnGCT
# 4 A2DChannel
# 5 ilakkiyakurangugal
# 6 InvestmentInsightsTamil

# CHANNEL NAMES 

# 1. Facts in 60s
# 2. Logus
# 3. MindYourLogic Tamil
# 4. Mr Brother
# 5. Muniba Mazari
# 6. Motivation Ark
# 7. RedFrost Motivation
# 8.The Puzzle House
# 9. With Love Shanthnu Kiki
# 10. Words of Wisdom
# 11. Education Analysist
...
if selected_query:
    if selected_query.startswith("1."):
        query = text("SELECT Video_Title, Channel_Name FROM video_info ORDER BY Channel_Name")
        ...
        query = text('''SELECT 
                        vi.Video_Title AS Video_Title, 
                        ch.Channel_Name AS Channel_Name, 
                        COUNT(ci.Comment_Id) AS Comment_Count
                        FROM 
                            video_info vi
                        JOIN 
                            comment_info ci ON vi.Video_Id = ci.Video_id
                        JOIN 
                            channel_info ch ON vi.Channel_Id = ch.Channel_id
                        GROUP BY 
